...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Save the animation to an MP4 file instead of opening a window.",
    )
    parser.add_argument(
        "--save-gif",
        type=Path,
        default=None,
        help="Save the animation to a GIF file instead of opening a window.",
    )
    ...
    if args.mode in ("animate", "both"):
        ...
        if args.save_mp4 is not None:
            # Headless render for static-site embedding.
            # Requires ffmpeg available in PATH.
            writer = FFMpegWriter(fps=30, bitrate=1800)
            anim.save(str(args.save_mp4), writer=writer)
            plt.close(fig)
        elif args.save_gif is not None:
            writer = PillowWriter(fps=30)
            anim.save(str(args.save_gif), writer=writer)
            plt.close(fig)
        else:
            plt.show()
